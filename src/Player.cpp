...
static const float PLAYER_JUMP_VELOCITY = std::sqrt(2.0f * GRAVITY * float(PLAYER_JUMP_HEIGHT));
static const float SPRING_JUMP_VELOCITY = PLAYER_JUMP_VELOCITY * std::sqrt(float(PLAYER_SPRING_MULTIPLIER));
...
void Player::init(const glm::ivec2 &tileMapPos, ShaderProgram &shaderProgram)
{
	...
	for (int f = 0; f < PLAYER_IDLE_FRAMES; ++f)
	{
		...
		float v = float(PLAYER_ROW_IDLE) / texH;
	...
	{
		float u = f * frameSize.x;
		float v = float(PLAYER_ROW_RUN) / texH;
	...
	{
		float u = f * frameSize.x;
		float v = float(PLAYER_ROW_JUMP) / texH;
	...
	{
		float u = f * frameSize.x;
		float v = float(PLAYER_ROW_HURT) / texH;
	...
	{
		float u = f * frameSize.x;
		float v = float(PLAYER_ROW_DEAD) / texH;
	...
	{
		float u = f * frameSize.x;
		float v = float(PLAYER_ROW_PROTECT) / texH;
...
void Player::update(int deltaTime)
{
	...
	{
		bProtecting = true;
		parryTimer = PLAYER_PARRY_FRAMES;
		parryCooldown = PLAYER_PARRY_COOLDOWN;
	...
	if (onDash && dashCooldown == 0) {
		...
		dashTimeLeftMs = PLAYER_DASH_DURATION_MS;
		dashVelocityStart = dashDir * (PLAYER_DASH_DISTANCE_BASE * 3.0f) / (0.5f * float(PLAYER_DASH_DURATION_MS));
	...
	{
		float ratio = float(dashTimeLeftMs) / float(PLAYER_DASH_DURATION_MS);
